...
| Codex | same names, same `hookSpecificOutput` shape | same — including the **Stop → plain-text** carve-out (codex Stop schema not yet live-captured, but the adapter mirrors Claude Code preemptively given every other shared shape matches). **TODO (codex Stop surfacing):** verify whether codex actually surfaces Stop-hook stdout to the next agent turn. Claude Code does not (live-verified 2026-06-17, see above) and codex's adapter mirrors Claude's shape preemptively, so the same silent-no-op gap is plausible. If contradicted, mirror the Claude workaround — also append to `journal.Pending` on Stop and drain via UserPromptSubmit `additionalContext` (one-line change in `internal/cli/nudge.go`). Recipe in `docs/verification/cross-harness.md`. | none — `PostToolUse` fires on success only; `PostToolFailureSensor = no` |
