...
**Fetched-ledger adoption:** a normal clone fetches the ledger as a remote-tracking ref such as `refs/remotes/origin/teammemory`, but does not necessarily create the local `refs/heads/teammemory` branch that the ledger plumbing reads. When the local ledger branch is missing, `tm` may adopt exactly one already-fetched remote-tracking ledger ref by creating the local branch at that commit. This repair is local-only: no network, no working-tree checkout, and no overwrite of an existing local branch. If multiple fetched remote ledger refs exist, `tm` refuses to guess and asks the user to create the branch explicitly.
...
**Orphan revival (cross-memory revert).** A `mark_duplicate` or `supersede` claim against a canonical A that is itself non-active (status `rejected`, or under an unresolved `mark_stale`) does **not** propagate its effect to the obsolete memory B. If A was previously active and B was `duplicate`/`superseded`, B reverts to its un-orphaned status when A becomes non-active. Computed in `derive.BuildContext` via the `Alive` map: A is "alive" iff it has no `reject` and no unresolved `mark_stale`. Operator-visible effect: `tm reject A` or `tm observe A mark_stale` on a canonical also revives any memories that were duplicates of A or superseded by A. The incremental index `Update` path fans out canonical-status changes to dependent memories so the materialized table stays consistent. Orphan revival is **reversible**: if a later `confirm` or human `approve` resolves a `mark_stale` on A (reviving A), every B that reverted is re-marked `duplicate`/`superseded` on the next derivation. `reject` is terminal, so a reject-driven revival is permanent.
...
Hook commands may perform the fetched-ledger adoption described in §7.1 before opening the SQLite index. This is still within the hook latency and safety model because it only inspects local refs and writes `refs/heads/teammemory` when there is exactly one already-fetched candidate; it never fetches from the network or checks out files.
...
**Stop hook** runs `tm nudge --hook`: at turn end, it reads the journal, detects the memory-worthy signals (fail→pass, revert, edit churn, surfaced-but-unobserved, drift-anchor), and emits at most one proposing/observing nudge per the anti-spam policy (max 3/session, cooldown 3 turns, suppress-if-already-acted, observe outranks propose). Low-pressure wording; the verbs stay voluntary. The nudge is injected as additional context, not as a forced turn. **Claude Code surfacing:** Stop-hook stdout does NOT reach the agent's next turn on Claude Code (live-verified 2026-06-17 against a session that fired three nudges without any reaching the transcript), so on the `claude` harness `tm nudge --hook` ALSO appends the rendered text to `journal.Pending`, and the next `tm signal --hook --prompt` (UserPromptSubmit) drains that queue via `hookSpecificOutput.additionalContext` — the channel verified to surface. **Codex surfacing:** Codex rejects plain-text Stop output and does not accept `hookSpecificOutput` for Stop advisory context, so Codex Stop nudges also append to `journal.Pending` and render nothing; the next UserPromptSubmit drains the queue via `additionalContext`. Other harnesses keep their adapter-specific context shape. Delivery is **at least once** across local journal persistence failures: direct-render attempts are persisted before output but do not consume dedup/budget until marked delivered, while queued text remains pending until a successful prompt render is saved. A failure saving that final delivered state can therefore cause a duplicate advisory rather than silently losing one.
...
`tm ack <memory-id> [--note "..."]` records an acknowledgment in `.git/tm/` (local-only). It is keyed by session ID when one is available, otherwise it falls back to a TTL (default 8h, configurable). Subsequent edits matching that memory pass while the ack is live. Acks are local state, not ledger records — they gate the hook, they are not evidence. **Session-id discovery:** `tm ack`/`tm propose`/`tm observe` resolve the session id in this order — explicit `--session <id>`, then `$CLAUDE_SESSION_ID`, then `.git/tm/current-session.txt` (written on each PostToolUse/Stop/UserPromptSubmit hook by `tm signal`/`tm nudge` from the payload's `session_id` field). The file fallback covers Claude Code, which carries `session_id` in the hook JSON payload but does NOT export `$CLAUDE_SESSION_ID` into the Bash tool's subprocess env (verified 2026-06-17, CLI 2.1.x) — without the file, every shelled-out `tm ack` would silently degrade to TTL semantics.
...
| --- | --- | --- | --- |
| Claude Code | `PreToolUse` / `PostToolUse` / `Stop` / `UserPromptSubmit` | `hookSpecificOutput.{permissionDecision,additionalContext}` for PreToolUse / PostToolUse / UserPromptSubmit; **Stop renders plain text to stdout** (Claude Code's Stop schema rejects `hookSpecificOutput`; live-verified 2026-06-16) BUT that stdout does NOT surface to the next agent turn (live-verified 2026-06-17, 3 fired nudges → 0 visible), so the adapter ALSO appends each nudge to `journal.Pending` and the next `UserPromptSubmit`'s `tm signal --hook --prompt` re-injects it via `additionalContext` | none — `PostToolUse` fires on success only (no failure event, no `exit_code`); `PostToolFailureSensor = no` |
| Codex | same names, same `hookSpecificOutput` shape | `hookSpecificOutput.{permissionDecision,additionalContext}` for PreToolUse / PostToolUse / UserPromptSubmit. Stop advisory output is silent and queued for the next UserPromptSubmit because Codex rejects plain-text Stop nudges and does not accept `hookSpecificOutput` for Stop advisory context (live-observed 2026-06-22). Defensive Stop blocks use top-level `{decision,reason}`. | none — `PostToolUse` fires on success only; `PostToolFailureSensor = no` |
...
3. **Provisional inclusion** (`provisional_mode`, default `related`): provisional and contested memories appear on any structural match (scope-glob or command-pattern), but not on FTS-only matches. Always caution-framed, always with a requested-observation prompt. Provisional and contested memories surface in **addition** to active memories, up to a separate `max_provisional` cap (default 2). They use a different budget from `max_results` (which caps active surfacing), so a hot area with 5+ active matches still receives caution-framed provisional context — that's exactly where new proposals are most likely, and starving them would stall the validation flywheel.
4. **Output cap:** `max_results` (default 5) active + `max_provisional` (default 2) provisional ⇒ at most 7 results. The two budgets are separate. Memories with status `duplicate`, `superseded`, `stale`, or `rejected` are excluded entirely. Context spam is a top product risk; the caps are a feature.
...
**Phase 3 — Memory trust & safety:** **Shipped.** Harden the trust gaps surfaced by external review — leak prevention, true independent confirmation, and bounded nudge cost. Lightweight local outcome instrumentation now provides the operational signal needed to tune nudge defaults, while a formal A/B capability evaluation remains deferred until there is enough product usage or a concrete product decision blocked by it.
