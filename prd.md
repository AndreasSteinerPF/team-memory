...
| Codex | same names, same `hookSpecificOutput` shape | same | `tool_response.exit_code` |
| Copilot | `preToolUse` / `postToolUse` / `errorOccurred` / `userPromptSubmitted` / `agentStop` | bare `{permissionDecision}` / `{additionalContext}` | `errorOccurred` event / `error` field / `toolResult.exitCode` (VERIFY exact field) |
...
**Packaging.** `tm init --harness {codex,copilot,cursor,gemini}` writes the harness's hook and plugin artifacts: Codex gets `<repo>/.codex/hooks.json` (the event map wrapped under a top-level `hooks` key — the path Codex actually loads; the legacy `.codex-plugin/` layout only loads as a marketplace-installed, trusted plugin) plus printed `codex mcp add` / `~/.codex/config.toml` guidance; Copilot gets `.github/hooks/teammemory.json` (each hook carrying both `bash` and `powershell` command keys so it runs cross-OS) plus printed `~/.copilot/mcp-config.json` guidance; Cursor gets `.cursor/hooks.json` plus `.cursor/rules/teammemory.mdc` plus `.cursor/mcp.json`; Gemini gets `.gemini/settings.json` (hooks + MCP) plus a `GEMINI.md` section. The default (`claude`) installs the Claude Code hooks into `.claude/settings.json` as before.
