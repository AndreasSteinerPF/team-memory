...
| --- | --- | --- | --- |
| Claude Code | `PreToolUse` / `PostToolUse` / `Stop` / `UserPromptSubmit` | `hookSpecificOutput.{permissionDecision,additionalContext}` for PreToolUse / PostToolUse / UserPromptSubmit; **Stop renders plain text to stdout** (Claude Code's Stop schema rejects `hookSpecificOutput`; live-verified 2026-06-16) | none — `PostToolUse` fires on success only (no failure event, no `exit_code`); `PostToolFailureSensor = no` |
