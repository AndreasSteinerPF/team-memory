...
| Codex | same names, same `hookSpecificOutput` shape | same — including the **Stop → plain-text** carve-out (codex Stop schema not yet live-captured, but the adapter mirrors Claude Code preemptively given every other shared shape matches) | none — `PostToolUse` fires on success only; `PostToolFailureSensor = no` |
